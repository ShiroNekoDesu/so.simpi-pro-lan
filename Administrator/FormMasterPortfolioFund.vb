...
Public Class FormMasterPortfolioFund
    ...
    Dim objFund As New MasterPortfolio
...
#Region "fund on fund"
    Private Sub FundLoad()
        If objPortfolio.GetPortfolioID > 0 Then
            objPortfolio.FOF_Load()
            If objPortfolio.ErrID = 0 Then
                lblFundCode.Text = objPortfolio.FOF_GetFundCode
                lblFundName.Text = objPortfolio.FOF_GetFundName
                lblClientCode.Text = objPortfolio.FOF_GetClientCode
                lblClientName.Text = objPortfolio.FOF_GetClientName
            Else
                lblFundCode.Text = ""
                lblFundName.Text = ""
                lblClientCode.Text = ""
    ...
    Private Sub SearchFund()
        Dim form As New FormSelectSimpiPortfolioNormal
        form.lblCode = lblFundCode
        form.lblName = lblFundName
        form.Show()
    ...
    Private Sub btnSearchUnitholder_Click(sender As Object, e As EventArgs) Handles btnSearchUnitholder.Click
        SearchClient()
    End Sub
    ...
    Private Sub btnSetFund_Click(sender As Object, e As EventArgs) Handles btnSetFund.Click
        FundSet()
    End Sub


    Private Sub FundSet()
        If objPortfolio.GetPortfolioID > 0 And lblFundCode.Text.Trim <> "" And lblClientCode.Text.Trim <> "" Then
            objFund.Clear()
            objFund.LoadCode(objMasterSimpi, lblFundCode.Text)
            If objFund.ErrID = 0 Then
                objClient.Clear()
                objClient.Load(objMasterSimpi, lblClientCode.Text)
                If objClient.ErrID = 0 Then
                    objPortfolio.FOF_Set(objFund, objClient)
                    If objPortfolio.ErrID = 0 Then
                        Close()
                    Else
                        ExceptionMessage.Show(objPortfolio.ErrMsg, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                ...
                End If
            Else
                ExceptionMessage.Show(objFund.ErrMsg, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        End If
    End Sub

    Private Sub btnRemoveFund_Click(sender As Object, e As EventArgs) Handles btnRemoveFund.Click
        FundRemove()
    End Sub

    Private Sub FundRemove()
        If lblFundCode.Text.Trim <> "" And lblClientCode.Text.Trim <> "" Then
            objPortfolio.FOF_Remove()
            If objPortfolio.ErrID = 0 Then
                Close()
            Else
                ExceptionMessage.Show(objPortfolio.ErrMsg, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
