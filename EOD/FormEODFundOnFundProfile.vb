...
Public Class FormEODFundOnFundProfile
    Public CallerForm As FormEODFundOnFund
    Public objFoF As MasterPortfolio
    Public navDate As Date
    ...
    Public Sub FormLoad()
        If objFoF.GetPortfolioID > 0 Then
            objFoF.FOF_Load()
            If objFoF.ErrID = 0 Then
                lblPortfolioCode.Text = objFoF.GetPortfolioCode
                lblPortfolioName.Text = objFoF.GetPortfolioNameFull
                lblSimpiName.Text = objFoF.GetMasterSimpi.GetSimpiName
                lblPortfolioCcy.Text = objFoF.GetPortfolioCcy.GetCcy
                lblInception.Text = objFoF.GetInceptionDate.ToString("dd-MMMM-yyyy")

                lblParentCode.Text = objFoF.FOF_GetFundCode
                lblParentName.Text = objFoF.FOF_GetFundName
                lblParentSimpiName.Text = objFoF.GetMasterSimpi.GetSimpiName
                lblParentSimpiEmail.Text = objFoF.GetMasterSimpi.GetSimpiName
                lblClientCode.Text = objFoF.FOF_GetClientCode
                lblClientName.Text = objFoF.FOF_GetClientName
            Else
                ExceptionMessage.Show(objFoF.ErrMsg, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Close()
            End If
    ...
    Private Sub FormEODFundOnFundProfile_Load(sender As Object, e As EventArgs) Handles MyBase.Load
