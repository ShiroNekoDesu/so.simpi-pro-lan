...
#End Region
    ...
    Private Sub ReportAccountValuation_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        GetPortfolioMaster()
        PortfolioID = (From p In dtPortfolioMaster Select p.Field(Of Integer)("PortfolioID")).ToArray
...
#Region "filter"
    ...
    Private Sub DisplaySummary()
        If dtTransaction IsNot Nothing AndAlso dtTransaction.Rows.Count > 0 Then
            Dim query = From u In dtFund.AsEnumerable Join p In dtPortfolioMaster.AsEnumerable
            ...
            Dim query2 = From u In dtTransaction.AsEnumerable Join p In dtPortfolioMaster.AsEnumerable
                         On u.Field(Of Integer)("PortfolioID") Equals p.Field(Of Integer)("PortfolioID")
                         Where p.Field(Of Integer)("CcyID") = cmbCcy.SelectedValue
...
#Region "investment growth"
    Private Sub DisplayAUM()
        If dtAUM IsNot Nothing AndAlso dtAUM.Rows.Count > 0 Then
            Dim query = From u In dtAUM.AsEnumerable Join p In dtPortfolioMaster.AsEnumerable
                        On u.Field(Of Integer)("PortfolioID") Equals p.Field(Of Integer)("PortfolioID")
                        Where p.Field(Of Integer)("CcyID") = cmbCcy.SelectedValue
...
#Region "pie asset"
    ...
    Private Sub chartAsset_MouseUp(sender As Object, e As MouseEventArgs) Handles chartAsset.MouseUp
        ...
        If e.Button.Equals(MouseButtons.Left) Then
            ...
            If [region].Equals(ChartRegionEnum.Legend) Then
                If chart.Legend.SeriesFromCoord(e.X, e.Y, g, s) Then
                    ...
                    If ser.Display.Equals(SeriesDisplayEnum.Show) Then ser.Display = SeriesDisplayEnum.Hide Else ser.Display = SeriesDisplayEnum.Show
                End If
                Return
            End If
            ...
            If grp.CoordToDataIndex(e.X, e.Y, CoordinateFocusEnum.XandYCoord, s, p, d) Then
                If d = 0 AndAlso s >= 0 AndAlso p >= 0 Then
                    ...
                    If offset = 0 Then offset = 40 Else offset = 0
                    ser.Offset = offset
                End If
            End If
        End If
    ...
    'Private Sub chartAsset_MouseMove(sender As Object, e As MouseEventArgs) Handles chartAsset.MouseMove
    ...
    '        If [region].Equals(ChartRegionEnum.Legend) Then
    '            If chart.Legend.SeriesFromCoord(e.X, e.Y, g, s) Then
    '                If g >= 0 AndAlso s >= 0 Then chart.Footer.Text = "Series #" + s.ToString()
    '            End If
    '        Else
    '            Dim grp As ChartGroup = chart.ChartGroups(0)
    '            If grp.CoordToDataIndex(e.X, e.Y, CoordinateFocusEnum.XandYCoord, s, p, d) Then
...
#Region "pie fund"
    ...
    Private Sub chartFund_MouseUp(sender As Object, e As MouseEventArgs) Handles chartFund.MouseUp
        Dim chart As C1Chart = CType(sender, C1Chart)
        If e.Button.Equals(MouseButtons.Left) Then
            Dim g As Integer = -1
            Dim s As Integer = -1
    ...
    'Private Sub chartFund_MouseMove(sender As Object, e As MouseEventArgs) Handles chartFund.MouseMove
    '    If e.Button.Equals(MouseButtons.None) Then
    '        Dim chart As C1Chart = CType(sender, C1Chart)
    '        Dim [region] As ChartRegionEnum = chart.ChartRegionFromCoord(e.X, e.Y)
    '        Dim g As Integer = -1
